...
module.exports = {
  add: (req, res) => {
    mongoose.connect(connUri, { useNewUrlParser : true, useUnifiedTopology: true }, (err) => {
      ...
      let status = 201;
      if (!err) {
        const { name, password, email, gender, phonenumber, role } = req.body;
        const user = new User({ name, password, email, gender, phonenumber, role}); // document = instance of a model
